...
const transformControlAcarreo = (items) => {
...
const transformControlMaterial = (items) => {
    if (!items || !Array.isArray(items))
        return [];
    return items.map(item => ({
        material: item.material,
...
const transformControlAgua = (items) => {
    if (!items || !Array.isArray(items))
        return [];
    return items.map(item => ({
...
const transformControlMaquinaria = (items) => {
    if (!items || !Array.isArray(items))
        return [];
    return items.map(item => ({
        tipo: item.tipo,
...
const transformPinesMapa = (items) => {
    if (!items || !Array.isArray(items))
        return [];
    return items.map(item => ({
        pin_id: item.id,
...
const transformPersonalAsignado = (items) => {
    if (!items || !Array.isArray(items))
        return [];
    return items.map(item => ({
...
const transformToSnakeCase = (data) => {
    ...
    for (const [key, value] of Object.entries(data)) {
        ...
        if (key === 'controlAcarreo') {
            transformed.controlAcarreo = transformControlAcarreo(value);
            continue;
        }
        if (key === 'controlMaterial') {
            transformed.controlMaterial = transformControlMaterial(value);
            continue;
        }
        if (key === 'controlAgua') {
            transformed.controlAgua = transformControlAgua(value);
            continue;
        }
        if (key === 'controlMaquinaria') {
            transformed.controlMaquinaria = transformControlMaquinaria(value);
            continue;
        }
        if (key === 'pinesMapa') {
            transformed.pinesMapa = transformPinesMapa(value);
            continue;
        }
        if (key === 'personalAsignado') {
            transformed.personalAsignado = transformPersonalAsignado(value);
            continue;
        }
